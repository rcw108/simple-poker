...
export const getBalance = expressAsyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (user.balance === undefined || user.balance === null) {
            ...
            await usersCollection.updateOne({ id: userIdNum }, { $set: { balance: 0 } });
            balance = 0;
        }
...
export const withdraw = expressAsyncHandler(async (req, res, next) => {
...
export const updateBalance = expressAsyncHandler(async (req, res) => {
    ...
    try {
        const usersCollection = db.collection('users');
        const result = await usersCollection.updateOne({ id: parseInt(String(userId)) }, { $set: { balance } });
...
export const verifyUser = expressAsyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (user) {
            ...
            if (user.balance === undefined || user.balance === null) {
                await usersCollection.updateOne({ id: userId }, { $set: { balance: 0 } });
                user.balance = 0;
            }
