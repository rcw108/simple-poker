...
async function connectMongoDB(retries = 5, delay = 3000) {
    for (let i = 0; i < retries; i++) {
        try {
            ...
            console.error(`Error connecting to MongoDB (attempt ${i + 1}/${retries}):`, errorMessage);
            if (i < retries - 1) {
                console.log(`Retrying in ${delay / 1000} seconds...`);
                await new Promise(resolve => setTimeout(resolve, delay));
            }
            else {
                console.error('Failed to connect to MongoDB after all retries. Server will continue but database operations will fail.');
        ...
        for (const tx of transactions) {
            if (tx.in_msg && tx.in_msg.value > 0) {
                ...
                if (user &&
                    ...
                    await usersCollection.updateOne({ id: parseInt(userId) }, {
