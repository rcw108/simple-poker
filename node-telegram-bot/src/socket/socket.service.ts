...
export const placeBet = (
	...
	socket: Socket,
	{
		...
		userId,
		betAmount,
	}: { roomId: string; userId: string; betAmount: number }
) => {
	const room = gameRooms[roomId]
...
export const dealCards = (
	...
	if (room.players.length !== 2)
		return socket.emit(
			'error',
			'The game requires exactly two players to start.'
		)

	if (room.stage !== 'initial')
		return socket.emit(
			'error',
			'Cannot deal cards. The game is not in the initial stage.'
		)

	if (!room.deck || room.deck.length === 0) room.deck = shuffleDeck()
...
export const revealCards = async (
	...
	const winner = results.reduce((best, current) =>
		current.hand.rank > best.hand.rank ? current : best
	)
	room.stage = 'reveal'
